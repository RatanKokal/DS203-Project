...
def create_dataset_from_csv(folder_path):
    ...
    for filename in os.listdir(folder_path):
        if filename.endswith('_MFCC.csv'):
            ...
            # Compute the RMS of the MFCCs
            rms_mfcc1 = np.sqrt(np.mean(np.square(mfcc_first_three[:, 0])))  # RMS of 1st MFCC
            rms_mfcc2 = np.sqrt(np.mean(np.square(mfcc_first_three[:, 1])))  # RMS of 2nd MFCC
            rms_mfcc3 = np.sqrt(np.mean(np.square(mfcc_first_three[:, 2])))  # RMS of 3rd MFCC
            
            # Combine the features into one list (total 13 features per file)
            file_features = feature_means + feature_stds + [rms_mfcc1, rms_mfcc2, rms_mfcc3]
    ...
    features_df = pd.DataFrame(features_list, columns=[
        ...
        'rms_mfcc1', 'rms_mfcc2', 'rms_mfcc3', 'Class'
    ])
