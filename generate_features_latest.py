...
def create_dataset_from_current_directory(window_size=30):
    ...
    for filename in os.listdir('.'):
        if filename.endswith('-MFCC.csv'):
            ...
            # Add the filename to the features list
            file_features.append(filename)
            
            # Append the features and filename for this file to the list
    ...
    output_file = './mfcc_features_with_filenames.csv'
    features_df.to_csv(output_file, index=False)
    
    print(f"Combined dataset saved to {output_file}")

# Example usage
create_dataset_from_current_directory(window_size=30)